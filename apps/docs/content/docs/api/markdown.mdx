...
| Tables | Native (markdown block) | Native GFM | ASCII fallback |
| Mentions | `<@USER>` | `<at>name</at>` | `<users/{id}>` |

<Callout type="info">
Slack accepts standard markdown via the `{ type: "markdown" }` Block Kit block on `chat.postMessage` and friends, so the SDK passes markdown through directly inside that block (with a plain-text fallback for notifications). The top-level `markdown_text` parameter is not used because it does not render GFM tables. Incoming Slack messages still arrive as legacy mrkdwn (`*bold*`, `<url|text>`) and are parsed transparently. If you need to send mrkdwn yourself, use `{ raw: "..." }`.

When you stream a `{ markdown }` message, Slack's streaming API only accepts incremental `markdown_text` chunks, so tables briefly appear as raw `|`-delimited text during streaming. The adapter rewrites the finalized message as a `markdown` block after streaming completes so tables render natively in their final form.
</Callout>
