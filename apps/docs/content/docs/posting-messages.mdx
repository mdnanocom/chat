...
Pass a `{ markdown }` object to have the SDK render standard markdown on each platform — emitted as a Slack `markdown` Block Kit block (which renders GFM tables, headings, lists, bold/italic, and code fences natively), converted to HTML for Teams, and so on.
