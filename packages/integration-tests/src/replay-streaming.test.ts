...
describe("Streaming Replay Tests", () => {
  describe("Slack", () => {
    ...
    it("should ignore a button prompt posted by the bot", async () => {
      await ctx.sendWebhook(slackFixtures.buttonPrompt);
    ...
    it("should stream structured chunks for a block_actions continuation", async () => {
      ...
      const hasStructuredAppend = streamer.append.mock.calls.some(
        ([payload]: [{ chunks?: Array<{ type?: string }> }]) =>
